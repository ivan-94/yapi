  ...
  _.each(res.paths, (apis, path) => {
    ...
    _.each(apis, (api, method) => {
      ...
      try {
        ...
        if (data.catname) {
          if (
            !_.find(interfaceData.cats, (item) => item.name === data.catname)
          ) {
      ...
      if (param.in) {
        switch (param.in) {
          ...
          case 'query':
            ...
            break;
          case 'body':
            handleBodyPamras(handleSchema(param.schema), api);
            break;
          case 'formData':
...
function handleSchema(schema) {
  ...
  const find = (root) => {
    ...
    if (Array.isArray(root)) {
      ...
      for (const key in root) {
        if (key === '$ref') {
          if (root[key].startsWith('#/definitions')) {
            const ref = root.originalRef || root[key].slice(14);
            set.add(ref);
          }
        } else {
          find(root[key]);
  ...
  if (set.size) {
    schema.definitions = {}
    set.forEach(v => {
      schema.definitions[v] =  SwaggerData.definitions[v]
    })
  }
...
function handleResponse(api) {
  ...
  if (codes.length > 0) {
    ...
    if (res && typeof res === 'object') {
      if (res.schema) {
        res_body = JSON.stringify(handleSchema(res.schema), null, 2);
